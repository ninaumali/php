...
try {
    ...
    $query = $connection->prepare("SELECT users.user_id, users.firstName, users.lastName, users.birthday, users.sex, users.user_name, users.user_profile_picture, CONCAT(user_add_city, ', ', user_add_province) AS address FROM users INNER JOIN user_address ON users.user_id = user_address.user_id LIMIT :offset, :recordsPerPage");
